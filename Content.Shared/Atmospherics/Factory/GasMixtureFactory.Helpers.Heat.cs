...
public sealed partial class GasMixtureFactory
{
    ...
    {
        m.Temperature = AddHeatQuery(ref m, dQ, SharedPool);
    }
    ...
    [PublicAPI]
    public void AddHeat<T>(ref T m, float dQ, IRobustArrayPool<float> pool) where T : IGasMixture
    {
        m.Temperature = AddHeatQuery(ref m, dQ, pool);
    }
    ...
    /// <exception cref="ArgumentOutOfRangeException">Thrown when the heat capacity of the container is zero or negative.</exception>
    [PublicAPI]
    public float AddHeatQuery<T>(ref T m, float dQ, IRobustArrayPool<float> pool) where T : IGasMixture
    {
        // Don't allow the temperature to go below the absolute minimum.
        return Math.Max(0f, m.Temperature + dQ / GetHeatCapacity(ref m, pool));
    }
}
