...
namespace Content.Client.UserInterface.Controls
{
    ...
    public sealed class MainViewport : UIWidget
    {
        ...
        public MainViewport()
        {
            ...
            _cfg.OnValueChanged(GameConfigVars.ViewportScalingFilterMode, _ => UpdateCfg(), true);
        }
        ...
        {
            var stretch = _cfg.GetCVar(GameConfigVars.ViewportStretch);
            var renderScaleUp = _cfg.GetCVar(GameConfigVars.ViewportScaleRender);
            var fixedFactor = _cfg.GetCVar(GameConfigVars.ViewportFixedScaleFactor);
            var verticalFit = _cfg.GetCVar(GameConfigVars.ViewportVerticalFit);
            var filterMode = _cfg.GetCVar(GameConfigVars.ViewportScalingFilterMode);
        ...
        private int? CalcSnappingFactor()
        {
            ...
            var cfgToleranceMargin = _cfg.GetCVar(GameConfigVars.ViewportSnapToleranceMargin);
            // Clip tolerance is tolerance of "the window is too small"
            // where we are clipping the viewport to make it fit.
            var cfgToleranceClip = _cfg.GetCVar(GameConfigVars.ViewportSnapToleranceClip);

            var cfgVerticalFit = _cfg.GetCVar(GameConfigVars.ViewportVerticalFit);
