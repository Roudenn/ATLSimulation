...
public sealed partial class GraphicsTab : Control
{
    ...
    public GraphicsTab()
    {
        ...
        Control.AddOptionDropDown(
            GameConfigVars.ViewportScalingFilterMode,
            DropDownFilterMode,
            [
        ...
        var vpStretch = Control.AddOptionCheckBox(GameConfigVars.ViewportStretch, ViewportStretchCheckBox);
        var vpVertFit = Control.AddOptionCheckBox(GameConfigVars.ViewportVerticalFit, ViewportVerticalFitCheckBox);
        Control.AddOptionSlider(
            GameConfigVars.ViewportFixedScaleFactor,
            ViewportScaleSlider,
            1,
            5,
        ...
        Control.AddOptionCheckBox(GameConfigVars.ViewportScaleRender, ViewportLowResCheckBox, invert: true);
        Control.AddOptionCheckBox(GameConfigVars.ParallaxLowQuality, ParallaxLowQualityCheckBox);
        Control.AddOptionCheckBox(GameConfigVars.HudFpsCounterVisible, FpsCounterCheckBox);

        Control.Initialize();

        _cfg.OnValueChanged(GameConfigVars.ViewportMinimumWidth, _ => UpdateViewportWidthRange());
        _cfg.OnValueChanged(GameConfigVars.ViewportMaximumWidth, _ => UpdateViewportWidthRange());
    ...
    {
        var min = _cfg.GetCVar(GameConfigVars.ViewportMinimumWidth);
        var max = _cfg.GetCVar(GameConfigVars.ViewportMaximumWidth);
    ...
    private sealed class OptionIntegerScaling : BaseOptionCVar<int>
    {
        ...
        protected override int Value
        {
            get => _checkBox.Pressed ? GameConfigVars.ViewportSnapToleranceMargin.DefaultValue : 0;
        ...
        public OptionIntegerScaling(
            ...
            IConfigurationManager cfg,
            CheckBox checkBox)
            : base(controller, cfg, GameConfigVars.ViewportSnapToleranceMargin)
        {
            _checkBox = checkBox;
            _checkBox.OnToggled += _ =>
            {
