...
public sealed partial class OptionsTabControlRow : Control
{
    ...
    [Dependency] private readonly ContentConfigurationManager _contentCfg = default!;
    ...
    public ServerOptionSliderFloatCVar AddServerOptionFloatSlider(
        CVarDef<float> cVar,
        OptionSlider slider,
        float min,
        float max,
        float scale,
        Func<ServerOptionSliderFloatCVar, float, string>? format = null)
    {
        return AddOption(new ServerOptionSliderFloatCVar(this, _cfg, _contentCfg, cVar, slider, min, max, scale, format ?? FormatFloat));
    }
    ...
    public ServerOptionSliderIntCVar AddServerOptionSlider(
        CVarDef<int> cVar,
        OptionSlider slider,
        int min,
        int max,
        Func<ServerOptionSliderIntCVar, int, string>? format = null)
    {
        return AddOption(new ServerOptionSliderIntCVar(this, _cfg, _contentCfg, cVar, slider, min, max, format ?? FormatInt));
    }
    ...
    private string FormatFloat(ServerOptionSliderFloatCVar slider, float value)
    {
        return $"{value:0.0}";
    }
    ...
    private static string FormatInt(ServerOptionSliderIntCVar slider, int value)
    {
        return value.ToString();
    }
}
...
/// <summary>
/// Derived class of <see cref="BaseOption"/> intended for making mappings to server CVars easier.
/// </summary>
/// <typeparam name="TValue">The type of the server-side CVar.</typeparam>
/// <remarks>
/// This works with the server side by executing commands with sudo rights.
/// </remarks>
/// <seealso cref="OptionsTabControlRow"/>
public abstract class BaseServerOptionCVar<TValue> : BaseOption
    where TValue : notnull
{
    /// <summary>
    /// Raised immediately when the UI value of this option is changed by the user, even before saving.
    ...
    private readonly ContentConfigurationManager _cfgSystem;
    private readonly IConfigurationManager _cfg;
    private readonly CVarDef<TValue> _cVar;
    ...
    protected BaseServerOptionCVar(
        OptionsTabControlRow controller,
        ContentConfigurationManager system,
        IConfigurationManager cfg,
        CVarDef<TValue> cVar)
        : base(controller)
    {
    ...
    public override void SaveValue()
    {
        _cfgSystem.SetCVar(_cVar, Value);
    }
...
/// </remarks>
/// <seealso cref="OptionsTabControlRow"/>
public sealed class ServerOptionCheckboxCVar : BaseServerOptionCVar<bool>
{
    private readonly CheckBox _checkBox;
    private readonly bool _invert;
    ...
    /// </para>
    /// </remarks>
    public ServerOptionCheckboxCVar(
        OptionsTabControlRow controller,
        IConfigurationManager cfg,
        ContentConfigurationManager system,
        CVarDef<bool> cVar,
        CheckBox checkBox,
        bool invert)
        : base(controller, system, cfg, cVar)
    {
        _checkBox = checkBox;
        _invert = invert;
        checkBox.OnToggled += _ =>
...
/// </summary>
/// <seealso cref="OptionsTabControlRow"/>
public sealed class ServerOptionSliderFloatCVar : BaseServerOptionCVar<float>
{
    /// <summary>
    /// Scale with which to multiply slider values when mapped to the backing CVar.
    /// </summary>
    ...
    private readonly OptionSlider _slider;
    private readonly Func<ServerOptionSliderFloatCVar, float, string> _format;

    protected override float Value
    {
        get => MathF.Round(_slider.Slider.Value * Scale, 1);
        set
        {
            _slider.Slider.Value = value / Scale;
            UpdateLabelValue();
    ...
    /// </param>
    /// <param name="format">Function that will be called to format the value display next to the slider.</param>
    public ServerOptionSliderFloatCVar(
        OptionsTabControlRow controller,
        IConfigurationManager cfg,
        ContentConfigurationManager system,
        CVarDef<float> cVar,
        ...
        float maxValue,
        float scale,
        Func<ServerOptionSliderFloatCVar, float, string> format) : base(controller, system, cfg, cVar)
    {
        Scale = scale;
        _slider = slider;
        _format = format;
...
/// </summary>
/// <seealso cref="OptionsTabControlRow"/>
public sealed class ServerOptionColorSliderCVar : BaseServerOptionCVar<string>
{
    private readonly OptionColorSlider _slider;
    ...
    /// <param name="cVar">The CVar that is being controlled by this option.</param>
    /// <param name="slider">The UI control for the option.</param>
    public ServerOptionColorSliderCVar(
        OptionsTabControlRow controller,
        IConfigurationManager cfg,
        ContentConfigurationManager system,
        CVarDef<string> cVar,
        OptionColorSlider slider) : base(controller, system, cfg, cVar)
    {
        _slider = slider;
...
/// </summary>
/// <seealso cref="OptionsTabControlRow"/>
public sealed class ServerOptionSliderIntCVar : BaseServerOptionCVar<int>
{
    private readonly OptionSlider _slider;
    private readonly Func<ServerOptionSliderIntCVar, int, string> _format;
    ...
    /// <param name="maxValue">The maximum value the slider should allow.</param>
    /// <param name="format">Function that will be called to format the value display next to the slider.</param>
    public ServerOptionSliderIntCVar(
        OptionsTabControlRow controller,
        IConfigurationManager cfg,
        ContentConfigurationManager system,
        CVarDef<int> cVar,
        ...
        int minValue,
        int maxValue,
        Func<ServerOptionSliderIntCVar, int, string> format) : base(controller, system, cfg, cVar)
    {
        _slider = slider;
        _format = format;
...
/// </summary>
/// <seealso cref="OptionsTabControlRow"/>
public sealed class ServerOptionDropDownCVar<T> : BaseServerOptionCVar<T> where T : notnull
{
    private readonly OptionDropDown _dropDown;
    private readonly ItemEntry[] _entries;
    ...
    /// <param name="dropDown">The UI control for the option.</param>
    /// <param name="options">The list of options shown to the user.</param>
    public ServerOptionDropDownCVar(
        OptionsTabControlRow controller,
        IConfigurationManager cfg,
        ContentConfigurationManager system,
        CVarDef<T> cVar,
        OptionDropDown dropDown,
        IReadOnlyCollection<ValueOption> options) : base(controller, system, cfg, cVar)
    {
        if (options.Count == 0)
            throw new ArgumentException("Need at least one option!");
